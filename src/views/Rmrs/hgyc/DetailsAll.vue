	...
	<div>
		<Row class="margin-top-10">
			<Col :lg="24">
	            <Card>
	                <p slot="title">
		                <Icon type="social-buffer"></Icon>明细一览
				    </p>
				    <Row>
				    	<Col span="2">
...
<script>
	...
	export default {
		    	...
		    	editInlineColumns: [
		    		{
				        ...
				        render:(h, params) => {
                            ...
                            const text = row.pm_id == null ? '-' : row.type != null ? row.type : '';
                            return h('span', {
                            }, text);
				        }
				    }, {
				        ...
				        render:(h, params) => {
				        	const row = params.row;
                            const text = row.degree == null ? '-' : row.degree != null ? row.degree : '';
                            return h('span', {
                            }, text);
				        }
				    }, {
				        ...
				        render:(h, params) => {
				        	const row = params.row;
                            const text = row.price == null ? '-' : row.price != null ? row.price : '';
                            return h('span', {
                            }, text);
				        }
				    }, {
	    ...
	    methods: {
	    	...
	    	Init () {
	    		const copy = this;
	    		this.$http.get(Api.url + 'hems/consumes/getAllConsumes/' + Api.rmId, {
	    			headers: {
	    	...
	    	Refer () {
	    		...
	    		this.$http.get(Api.url + 'hems/consumes/getAllConsumes/' + Api.rmId + '/' + start + '/' + stop, {
	    			headers: {
						Authorization: "Bearer " + localStorage.getItem("access_token")
					}
	    		})
	       ...
	       //导出数据Csv格式
			...
			//导出数据xls格式
			exportExcel() {
				table2excel.transform(this.$refs.table, 'hrefToExportTable', '明细一览');
			},
	    }
	}
