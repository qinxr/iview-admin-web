	...
	<div>
		<Row class="margin-top-10">
			<Col :lg="24">
	            <Card>
	                <p slot="title">
		                <Icon type="social-buffer"></Icon>电表扣费明细
				    </p>
				    <Row>
				    	<Col span="2">
...
<script>
	...
	export default {
	    ...
	    methods: {
	    	...
	    	Init () {
	    		const copy = this;
	    		this.$http.get(Api.url + 'hems/consumes/getChargeConsumes/' + Api.rmId, {
	    			headers: {
	    	...
	    	Refer () {
	    		...
	    		this.$http.get(Api.url + 'hems/consumes/getChargeConsumes/' + Api.rmId + '/' + start + '/' + stop, {
	    			headers: {
						Authorization: "Bearer " + localStorage.getItem("access_token")
					}
	    		})
	       ...
	       //导出数据Csv格式
			...
			//导出数据xls格式
			exportExcel() {
				table2excel.transform(this.$refs.table, 'hrefToExportTable', '电表扣费明细');
			},
	    }
	}
