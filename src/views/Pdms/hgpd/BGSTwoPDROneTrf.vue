	...
	export default {
		...
		data() {
			return {
				...
				searchConName1: "",
				svg: {
					'src': require("../../../images/ACLP/54.svg")
				},
				trfData: [],
				cNamedata1: [{
					id: '',
		...
		methods: {
			...
			Init() {
				...
				const copy = this;
				//初始化页面的数据
				this.$http.get(Api.url + 'pdms/monitor/getMonitorAsTrf/23', {
						headers: {
					...
					.then(response => {
						...
						for(var i = 0; i < copy.svgData.length; i++) {
							if(i < num) {
								...
								if(copy.svgData[i].pm_type == 3) {
								...
								}else if(copy.svgData[i].pm_type == 7){
									...
									if(copy.svgData[i].status == 1) {
										copy.cValuedata1[1].y = 157 + 20 + (i - 1) * 66;
										copy.cValuedata1[1].value = copy.svgData[i].i;
										copy.cValuedata1[1].color = "blue";
								...
								if(copy.svgData[i].pm_type == 3) {
									...
									if(copy.svgData[i].status == 1) {
										...
										copy.cValuedata2[1].y = 92 + 20 + (i - num) * 66;
										copy.cValuedata2[1].value = copy.svgData[i].i;
										copy.cValuedata2[1].color = "blue";
