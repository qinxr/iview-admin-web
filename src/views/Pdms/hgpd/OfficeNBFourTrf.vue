	...
	export default {
		...
		data() {
			return {
				...
				searchConName1: "",
				svg: {
					'src': require("../../../images/ACLP/50.svg")
				},
				trfData: [],
				cNamedata1: [{
					id: '',
		...
		methods: {
			...
			Init() {
				...
				const copy = this;
				//初始化页面的数据
				this.$http.get(Api.url + 'pdms/monitor/getMonitorAsTrf/4', {
						headers: {
						...
						for(var i = 0; i < copy.svgData.length; i++) {
							if(i < num) {
								...
								if(copy.svgData[i].pm_type == 3) {
									...
									copy.cNamedata2[1].y = 74 + (i - num) * 66;
									copy.cNamedata2[2].id = 'cib' + i;
									copy.cNamedata2[2].y = 74 + 20 + (i - num) * 66;
									copy.cNamedata2[3].id = 'cic' + i;
									copy.cNamedata2[3].y = 74 + 40 + (i - num) * 66;

									//数值绑定
									if(copy.svgData[i].status == 1) {

										copy.cValuedata2[0].y = 74 + (i - num) * 66;
										copy.cValuedata2[0].value = copy.svgData[i].ia;
										copy.cValuedata2[0].color = "blue";

										copy.cValuedata2[1].y = 74 + 20 + (i - num) * 66;
										copy.cValuedata2[1].value = copy.svgData[i].ib;
										copy.cValuedata2[1].color = "blue";

										copy.cValuedata2[2].y = 74 + 40 + (i - num) * 66;
