            ...
            <Card>
                <p slot="title">
	                <Icon type="ivu-icon ivu-icon-alert-circled"></Icon>
				           电表告警订阅
				    <router-link to="/Pdms/hbgs/projectManagement/ammeterWarningAdd" class="pdRoom-add">
                ...
                <div class="edittable-table-height-con">
                    <Table border refs="table" :data="ammWarningData" v-model="ammWarningData"  :columns="editInlineColumns" ></Table>
                </div>
...
<script>
	export default {
		...
		mounted () {
			const copy = this;
			//获取电表告警信息
			this.$http.get(Api.url + 'pdms/pmWarningSub/getPmWarningSubAsProject/' + Api.hbgsId, {
				headers: {
		...
		methods: {
			...
			Remove () {
				const copy = this;
				this.$http.post(Api.url + 'pdms/pmWarningSub/deletePmWarningSub', {
					pmWarningSubId: copy.ammWarningId
				}, {
					headers: {
						Authorization: "Bearer " + localStorage.getItem("access_token")
					}
