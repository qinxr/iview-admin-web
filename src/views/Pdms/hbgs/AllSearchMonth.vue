<template>
	<div>
		<Row class="margin-top-10">
			...
			<Card>
				<p slot="title">
					<Icon type="social-buffer"></Icon>月度用电量
				</p>
				<Row>
					...
					<Row>
						...
						<Col span="4" class="start-time">
						<DatePicker type="month" :options="options1" placeholder="选择日期" style="width: 200px" v-model="startTime"></DatePicker>
						</Col>
						<Col span="2">
						<span class="time">结束时间：</span>
						</Col>
						<Col span="4">
						<DatePicker type="month" :options="options1" placeholder="选择日期" style="width: 200px" v-model="stopTime"></DatePicker>
						</Col>
	...
	export default {
		...
		methods: {
			...
			Refer() {
				...
				this.$http.get(Api.url + 'pdms/pmMonthDegree/getPmMonthDegreeDatas/' + start + '/' + stop + '/' + copy.type + '/' + id, {
						headers: {
							Authorization: "Bearer " + localStorage.getItem("access_token")
						}
					})
