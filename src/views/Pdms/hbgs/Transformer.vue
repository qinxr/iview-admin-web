	...
	<Row class="margin-top-10">
		...
		<Card>
			<p slot="title">
				...
				变压器管理
				<Button type="info" style="margin-left: 20px;"><router-link to="/Pdms/hbgs/projectManagement/transformerAdd" class="user-add"><Icon type="plus-round"></Icon>&nbsp;&nbsp;变压器新增</router-link></Button>
			</p>
...
<script>
	...
	export default {
		...
		mounted() {
			const copy = this;
			//获取全部变压器
			this.$http.get(Api.url + "pdms/transformers/getTransformerAsProject/" + Api.hbgsId, {
				headers: {
		...
		methods: {
			...
			del() {
				...
				axios.post(Api.url + "pdms/transformers/deleteTransformer", {
					transformerId: id
				}, {
					headers: {
						Authorization: "Bearer " + localStorage.getItem("access_token")
