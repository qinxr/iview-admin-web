	...
	export default {
		...
		methods: {
			Init () {
				...
				//根据分区回水Id获取信息
				this.backWaterID = this.$route.params.id;
				this.$http.get(Api.url + 'hvac/hexTrmns/getHexTrmn/' + this.backWaterID, {
					headers: {
						Authorization: "Bearer " + localStorage.getItem("access_token")
					},
				})
				.then(response => {
					//console.log(response.data.data);
					copy.backWaterForm = response.data.data;
				})
				.catch(error => {
					console.log(error);
				});
			...
			Edit () {
				...
				if(this.backWaterForm.channel_id == '' || this.backWaterForm.name == '' || this.backWaterForm.hex_id == '') {
					...
					this.$http.post(Api.url + 'hvac/hexTrmns/editHexTrmn/' + this.backWaterID, {
