            ...
            <Card>
                <p slot="title">
	                <Icon type="social-buffer"></Icon>
				          分区回水管理
				    <router-link to="/Hvac/tzwl/backWaterAdd" class="hexs-add">
                ...
                <div class="edittable-table-height-con">
                    <Table border refs="table" :data="backWaterData" v-model="backWaterData"  :columns="editInlineColumns" ></Table>
                </div>
...
<script>
	export default {
		...
		mounted () {
			const copy = this;
			//获取分区回水信息
			this.$http.get(Api.url + 'hvac/hexTrmns/getHexTrmnAsProject/' + Api.tzwlId, {
				headers: {
			...
			.then(response => {
                ...
                if(copy.historyData.length < copy.pageSize){
                    copy.backWaterData = copy.historyData;
                }else{
                    copy.backWaterData = copy.historyData.slice(0,copy.pageSize);
                }
			})
			.catch(error => {
				console.log(error);
		...
		methods: {
			...
			changePage(index) {
                ...
                this.backWaterData = this.historyData.slice(_start,_end);
			},
			//删除分区回水信息
			Remove () {
				const copy = this;
				this.$http.post(Api.url + 'hvac/hexTrmns/deleteHexTrmn', {
					hexTrmnId: copy.backWaterId
				}, {
					headers: {
						Authorization: "Bearer " + localStorage.getItem("access_token")
					}
