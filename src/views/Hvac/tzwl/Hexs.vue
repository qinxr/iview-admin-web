            ...
            <Card>
                <p slot="title">
	                <Icon type="social-buffer"></Icon>
				          分、集水器管理
				    <router-link to="/Hvac/tzwl/hexsAdd" class="hexs-add">
                ...
                <div class="edittable-table-height-con">
                    <Table border refs="table" :data="hexsData" v-model="hexsData"  :columns="editInlineColumns" ></Table>
                </div>
	...
	export default {
				...
				editInlineColumns: [
				    ...
				    },
				    {
				        ...
				        render:(h, params) => {
                            ...
                            const text = row.type == 1 ? '分水器' : row.type == 2 ? '集水器' : '';
                            return h('span', {
                            }, text);
				        }
				    },
		...
		mounted () {
			...
			.then(response => {
                ...
                if(copy.historyData.length < copy.pageSize){
                    copy.hexsData = copy.historyData;
                }else{
                    copy.hexsData = copy.historyData.slice(0,copy.pageSize);
                }
			})
			.catch(error => {
				console.log(error);
		...
		methods: {
			...
			changePage(index) {
                ...
                this.hexsData = this.historyData.slice(_start,_end);
			},
			//删除分、集水器信息
			Remove () {
				const copy = this;
				this.$http.post(Api.url + 'hvac/hexs/deleteHex', {
					hexId: copy.hexsId
				}, {
					headers: {
						Authorization: "Bearer " + localStorage.getItem("access_token")
