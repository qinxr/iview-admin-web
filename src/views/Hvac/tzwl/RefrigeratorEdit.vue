...
<script>
	export default {
		...
		methods: {
			...
			Init () {
				const copy = this;
				this.refrigeratorId = this.$route.params.id;
				this.$http.get(Api.url + 'hvac/refrigerators/getRefrigerator/' + this.refrigeratorId, {
					headers: {
						Authorization: "Bearer " + localStorage.getItem("access_token")
					}
				})
				.then(response => {
					copy.refrigeratorForm = response.data.data;
				})
				.catch(error => {
					console.log(error);
				})
			...
			Edit () {
				...
				if(this.refrigeratorForm.channel_id == '' || this.refrigeratorForm.name == '') {
					...
					this.$http.post(Api.url + 'hvac/refrigerators/editRefrigerator/' + this.refrigeratorId, {
